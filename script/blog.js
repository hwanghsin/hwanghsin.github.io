...
$(document).ready(async function() {
        ...
        switch (type) {
            ...
            case 'save':
                ...
                if (tempVerseArr.length > 0) {
                    const verArrRes = await insertVerse({ token, verses: tempVerseArr, blogId: currentBlogId });
    ...
    $('#save').on('click', function() {
        ...
        if (!title) {
            ...
            showToast(`請輸入文章主題`);
            return;
        } else { $('#title').removeClass('border-danger'); }
        if (!blogtype) {
            $('#category').addClass('border-danger');
            $(this).prop('disabled', false);
            showToast(`請選擇文章分類`);
            return;
        } else { $('#category').removeClass('border-danger'); }
        $('#confirmation').modal('show');
        $('#confirmation').find('.modal-body > h4').text('確認送出表單？');
        $('#confirmation').find('#submit').text('確認送出');
        $('#confirmation').find('#submit').removeClass('btn-danger').addClass('btn-success')
        $('#confirmation').find('#submit').attr('name', 'save');
    });
    $('#reset').on('click', function() { clearForm(); });
    ...
    $('#preview-verse').on('click', async function() {
        ...
        $(this).prop('disabled', false);
        if (verseResponse.status !== 'success') return showToast('取得書卷出現問題');
        const title = `${BIBLE_VERSES.find(v => v.code === chineses).name} 第${chap}章`;
        const str = verseResponse.record.map(r => {
            return `${r.sec} ${r.bible_text}`;
        }).join(' ');
        $('#verse-content').empty().append(`<i class="position-absolute fas fa-times fa-lg times" id="close-verse"></i><h4>${title}</h4><p>${str}</p>`).addClass('extended');
    });
    $('#add-verse').on('click', async function() {
        ...
        $('#verse-container').prepend(renderVerseCard({ book, verse: { chap, sec, id: '' }})); 
        $('#chineses').val('');
        $('#chap').val('');
        $('#sec').val('');
        $(this).prop('disabled', false);
    });
    $(document).on('click', '.get-id-icon', function() { showToast(`文章ID已複製`); });
    $(document).on('click', '.delete-icon', function() {
        ...
        $('#confirmation').find('#submit').removeClass('btn-success').addClass('btn-danger');
    ...
    $(document).on('click', '#blog-author', function() {
        ...
        [...$(this).parent().children()].forEach(el => {
            $(el).find('.badge').attr('class', `badge text-white bg-${$(el).attr('name') === name ? 'success' : 'secondary'}`);
        });
        $('#blog-list').empty();
        [...blogs].filter(item => item.updatedBy === name).reverse().forEach(renderList);
    });
    ...
    function renderVerseCard({ book, verse }) {
        ...
        <div class="card" style="width: 10rem;">
            <div class="card-body px-3 py-2">
                ...
                <h6 class="card-subtitle mb-2 text-muted">${verse.sec ? `第${verse.sec}節` : ''}</h6>
                <a class="card-link" href="#" id="delete-verse" name="${verse.id}">刪除</a>
            </div>
        </div>
        `
    }
...
async function updateBlog({ token, blog }) {
    ...
    const res = await (await fetch(`${url}/api/blogs/${blog.id}`, options)).json();
    return res;
}
...
async function fetchBibleVerses({ chineses = '羅', chap = 1, sec = '' }) {
    const res = await (await fetch(`https://bible.fhl.net/json/qb.php?chineses=${chineses}&chap=${chap}&sec=${sec}`, options)).json();
    return res;
}
...
async function insertVerse({ token, verses, blogId }) {
    ...
    const res = await (await fetch(`${url}/api/verse/blog/${blogId}`, options)).json();
    return res;
}
