...
$(document).ready(async function() {
    ...
    $('#add-click').on('click', async function() {
        ...
        $('#submit').attr('name', 'new').removeClass('btn-danger').addClass('btn-success').text('新增');
    ...
    $('#confirmation').on('hide.bs.modal', function () { clearForm(); });
    $('#back-click').on('click', function () {
        const id = $(this).attr('name');
        const prayerTarget = allList.find(item => item.id === id);
        currentParent = prayerTarget.parent || null;
        $('#parent').text(prayerTarget.parent ? allList.find(item => item.id === prayerTarget.parent).target : '禱告清單');
    ...
    $(document).on('focus', '#content', function() { $(this).removeClass('border-danger'); });
    $(document).on('click', '#edit-click', async function() {
        ...
        $('#confirmation').find('.modal-body').empty();
        $('#submit').attr('name', 'edit').removeClass('btn-danger').addClass('btn-success').text('修改');
        createForm(user);
    });
    $(document).on('click', '#ans-click', async function() {
        currentId = $(this).parent().attr('name');
        $('#confirmation').find('.modal-body').empty();
        $('#submit').attr('name', 'remove').removeClass('btn-success').addClass('btn-danger').text('移除');
        $('#confirmation').modal('show').find('.modal-body').append(`<h4>確定移除此項目？</h4>`).parent().find('.modal-footer > .form-check').addClass('d-none');
    });
    ...
    function insertPrayerCard({ id, title = '', content, target = '' }) {
        ...
        <div class="card mb-3">
            <div class="card-body">
                <h5 class="card-title"><span class="font-weight-bold">${title}</span> ${target ? `(對象：${target})` : ''}</h5>
                <p class="card-text">${content}</p>
                <div class="btn-group mr-2" role="group" name="${id}">
                    <button type="button" class="btn btn-secondary" id="edit-click">編輯</button>
                    ${target && '<button type="button" class="btn btn-secondary" id="sub-click">細節</button>'}
    ...
    function renderList() {
        ...
        list.forEach(item => { insertPrayerCard({ ...item }); });
    }
    function clearForm() {
        $('#title').val('');
        $('#target').val('');
...
async function deletePrayer({ id, token }) {
    ...
    const res = await (await fetch(`${url}/api/prayer/answer/${id}`, options)).json();
    return res;
}